...
setup-local: tools
	@docker-compose up -d
	@sleep 5
	@docker exec -it gotes-mx-pg psql -h localhost -p 5757 -U $(DB_USER) -tc "SELECT 1 FROM pg_database WHERE datname = '$(DB_NAME)'" | grep -q 1 || (docker exec -it gotes-mx-pg psql -h localhost -p 5757 -U $(DB_USER) -c "CREATE DATABASE $(DB_NAME)" && echo "Database $(DB_NAME) created")
